...
import argparse
def get_args():
    p = argparse.ArgumentParser("VSNet multitask training — robust v5")
...
def main():
    ...
    model.to(device)
    # v5: do not set model memory_format; only set channels_last_3d on inputs
        ...
        if epoch % args.val_every == 0 or epoch == args.epochs:
            ...
            with torch.no_grad():
                for vb in val_loader:
                    ...
                    if args.channels_last:
                        vx = vx.contiguous(memory_format=torch.channels_last_3d)
                    with torch.autocast(device_type="cuda", enabled=args.fp16):
                        logits = model(vx)[0]
